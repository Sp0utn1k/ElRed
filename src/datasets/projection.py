...
def get_projection(lats, lons):
    ...
    proj = Proj(proj="ortho", lat_0=lat0, lon_0=lon0, datum="WGS84")
...
def validate_projection(latlon_data, proj, tol=0.03):
    ...
    if meridional_error_pct > tol:
        warning_msg.append(
            f"Meridional error {meridional_error_pct:.2%} exceeds tolerance {tol:.2%}. "
        )
    if parallel_error_pct > tol:
        warning_msg.append(
            f"Parallel error {parallel_error_pct:.2%} exceeds tolerance {tol:.2%}. "
        )
    if angular_error_pct > tol:
        warning_msg.append(
            f"Angular error {angular_error_pct:.2%} exceeds tolerance {tol:.2%}. "
        )
